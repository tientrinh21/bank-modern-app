          ...
          <div key={footerLink.title} className="flex flex-col ss:my-0 my-4 min-w-[150px]">
            ...
            <ul className="mt-4">
              {footerLink.links.map((link) => (
                <li
                  key={link.name}
                  className="font-poppins font-normal text-[16px] leading-[24px] text-dimWhite hover:text-secondary cursor-pointer mb-4 last-of-type:mb-0"
                >
                  {link.name}
                </li>
              ))}
    ...
    <div className="w-full flex justify-between items-center flex-col md:flex-row pt-6 border-t-[1px] border-t-[#3f3e45]">
      ...
      <div className="flex flex-row md:mt-0 mt-6">
        {socialMedia.map((social) => (
          <img key={social.id} src={social.icon} alt={social.icon} className="w-[21px] h-[21px] object-contain cursor-pointer mr-6 last-of-type:mr-0" />
        ))}
      </div>
    </div>
  </section>
